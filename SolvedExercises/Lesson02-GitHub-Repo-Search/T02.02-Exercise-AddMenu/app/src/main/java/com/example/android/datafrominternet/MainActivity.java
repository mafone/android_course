...
public class MainActivity extends AppCompatActivity {
    ...
    //On res -> File -> New folder (menu) -> on this folder ->
    ...
    //First we override onCreateOptionsMenu
    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.main, menu); //Inflating the menu here
        return true;
    ...
    //Second we override onOptionsItemSelected

    @Override
    public boolean onOptionsItemSelected(MenuItem item) { //Function which handles the selection of any menu item
        //We can call a Toast here
        int menuItemThatWasSelected = item.getItemId(); //Return the selected item by user
        if (menuItemThatWasSelected == R.id.action_search) { //Is our menu item the selected one? For several menu items, we would use a switch
            //Let's open a Toast in the current contest when our item is selected on the menu
            Context context = MainActivity.this; //Contest where the toast'll be shown
            String message = "Item Search clicked"; //Message to show on toast
            Toast.makeText(context, message, Toast.LENGTH_LONG).show(); //Show the toast
        }
